  ...
  Decorations = new Class(Repo, {
    ...
    decorate: function(dataObj, model){
      ...
      if (!model) {
        if (arguments.length === 1) {
          return this.decorateWHash(dataObj);
        }
      ...
      return this.applyDecoration(dataObj, decorator);
    },
    marshal: function(dataObj, model){
      return this.decorate(dataObj, model);
    },
    decorateWHash: function(hash){
      var self, keys, decorated;
      self = this;
      keys = _.keys(hash);
      if (keys.length === 1) {
        decorated = null;
        _.keys(hash).each(function(key){
          var decorated;
          return decorated = self.decorate(hash[key], key);
        });
        return decorated;
      }
    ...
    applyDecoration: function(dataObj, decorator){
      if (typeof decorator === 'object') {
        this.applyDecorationBlueprint(dataObj, decorator);
      }
